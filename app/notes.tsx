...
export default function NotesProScreen() {
  ...
  const moveNoteToFolder = async (noteId: string, folderId: string | null) => {
    try {
      ...
      console.error(e);
      Alert.alert("Error", "Could not move note.");
    }
  };
  ...
  const TreeMicroTopic = ({ topic, sectionId }: { topic: PilotVaultMicroTopic, sectionId: string }) => {
    ...
    const animatedStyle = useAnimatedStyle(() => ({
      transform: [{ scale: withSpring(isHovered.value ? 1.05 : 1) }],
            ...
            <View style={styles.actionsContainer}>
              ...
              <TouchableOpacity 
                ...
                onPress={() => {
                  ...
                  setTargetSubject(actionNote?.subject || null);
                  setTargetSection(actionNote?.section_group || '');
                  setActionNote(null);
                  setMoveNoteVisible(true);
                }}
              >
